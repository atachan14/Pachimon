...
namespace Pachimon.Editor.UI
{
    public static class DescriptionTemplateBatch07Setup
    {
        ...
        private static string CreateSkillTemplate(int id) => id switch
        {
            ...
            55 => "{value:hitCount}体へ連鎖し、各対象へ{color:Wind}{value:damage}{/color}（{value:baseDamage} ×（100 + {icon:Wind}{value:wind} × {value:damageRatio}%）%）の{icon:Wind}{color:Wind}ダメージ{/color}と値{value:erosion}（{value:baseErosion} ×（100 + {icon:Wind}{value:wind} × {value:erosionRatio}%）%）の風化を与える。使用後、きりきり舞いの追加連鎖数が{value:chainGain}増加する。",

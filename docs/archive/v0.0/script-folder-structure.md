...
目的は、実装が進んだあとも `App / UI / Battle / Map / Run / StaticData / Editor` の責務が混ざらないようにすること。
...
### `Assets/Scripts/Runtime/StaticData`
- code-first の静的データ置き場
- `PachimonInfoTable`
- `GlobalStatGain`
- 必要なら `Trainer / GymLeader / Mod` の catalog

## Editor
### `Assets/Scripts/Editor`
- 今は空でよい
- 将来、専用 editor tool が必要になったら追加する
...
- State: `XxxState`
- Context: `XxxContext`
- Catalog / Table: `XxxCatalog` / `XxxTable`

## 補足
- まだファイル数が少ないうちは、細かく分けすぎない
- ただし `App` と `UI` と `Battle` と `Map` と `Run` と `StaticData` と `Editor` は最初から分ける

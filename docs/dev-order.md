...
このプロジェクトは、当面 `DefinitionTable -> Run / Map -> Node -> Battle` の順で進める。
理由は、`MapGenerator` が `Pachimon / Mod / Trainer / GymLeader` の定義を前提にするため。
特に `row:0 の初期パチモン選択` を含めるなら、先にデータ責務を固めたほうが後の手戻りが少ない。

## 方針
- `TopScene -> GameScene` の流れは維持する
- その上で、先に `DefinitionTable` と `RunState` の責務境界を固める
- `DefinitionTable` の schema と制作を先に進める
- `Map生成` は Definition を使う前提で本実装寄りに進める
...
目的:
- `DefinitionTable` と `RunState` の責務境界を固定する

やること:
- `Definition` / `Instance` / `RunState` / `RunContext` の役割整理
- player の現在パーティを `RunState` に持たせる前提を固定
- `SkillDefinitionTable` / `PassiveDefinitionTable` を本当に持つか再整理
- [domain-boundaries.md](./domain-boundaries.md) を正とする

完了条件:
- `誰が何を持つか` が docs 上で迷わない

## Phase 1: DefinitionTable 構造構想
目的:
...
完了条件:
- CSV 列案と Unity 側の row / table 形が決まっている

## Phase 2: DefinitionTable 制作
目的:
- 実際に扱う定義データを作り始める
...
## いまの優先順位
1. `Domain Boundary`
2. `DefinitionTable` 構造構想
3. `DefinitionTable` 制作

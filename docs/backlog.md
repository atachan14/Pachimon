...
- 戦闘不能フェードアウト後、Titleにフェードイン
- battleのUX/UI
    - log送りを別のもの（「おう」ボタンじゃないもの）にする。
        - ボタンじゃなく、右下に「▼」とかを表示（わかりやすい方法を検証）
    - logに表示する内容の精査。
    - Skillクリックでプレビュー、二回目のクリックで決定にする。


- Skill/Passive詳細
    - 
- SkillやPassiveを追加していくにあたって、151体という前提を一旦解除して、実装分だけでMapを生成できるようロジックを組み換え、将来的に151体を目指すという案。

- 戦闘中にMapを開いて閉じると、RightPane内の表示がなくなる。
- StartNodeでのRightPaneのPachimonTabでもSkill/Passive詳細を開けるようにして～～
...
ただし、対象範囲は制限したほうがよさそうです。四天王は最初から公開されているため、公開済みNodeすべてを対象にするとStart直後から石を投げられます。ただし、対象範囲は制限したほうがよさそうです。四天王は最初から公開されているため、公開済みNodeすべてを対象にするとStart直後かただし、対象範囲は制限したほうがよさそうです。四天王は最初から公開されているため、公開済みNodeすべてを対象にするとStart直後から石を投げられます。ただし、対象範囲は制限したほうがよさそうです。四天王は最初から公開されているため、公開済みNodeすべてを対象にするとStart直後から石を投げられます。ら石を投げられます。
進捗と仮の制作順:
